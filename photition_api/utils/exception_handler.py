...
def custom_exception_handler(
    ...
    # handle ServiceObjects library  exceptions
        ...
        for field, exception_details in exception.errors.items():
            detail_list = []
            for exception_detail in exception_details:
                if hasattr(exception_detail, "error_dict"):
                    for (
                        field_name,
                        error_details,
                    ) in exception_detail.error_dict.items():
                        inner_detail_list = []
                        for error_detail in error_details:
                            for detail in error_detail.error_list:
                                for message in detail.messages:
                                    inner_detail_list.append(
                                        {
                                            "translation_key": detail.code,
                                            "message": message,
                                        }
                                    )
                        detail_list.append({field_name: inner_detail_list})
                else:
