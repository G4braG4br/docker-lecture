...
class ModeratePhotoTestCase(PhotoAPITestCase):
    def setUp(self):
        self.user = PhotitionUserFactory.create(is_staff=True)
        ...
        self.url = reverse(
            "photo", query={"action": "approve"}, kwargs={"id": self.photo.id}
        )

    def test_moderate_photo_200(self):
